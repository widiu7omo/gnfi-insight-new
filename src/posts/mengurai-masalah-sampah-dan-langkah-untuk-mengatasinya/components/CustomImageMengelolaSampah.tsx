...
import { motion, type Variants, MotionConfig, useInView } from "motion/react";
import NumberFlow, { useCanAnimate } from '@number-flow/react'
import { useEffect, useRef, useState } from "react";
const MotionNumberFlow = motion(NumberFlow)
export function CustomImageMengelolaSampah() {
  ...
  return (
    <div ref={ref} className="relative overflow-clip font-sora py-14 bg-no-repeat bg-cover" style={{ backgroundImage: `url(/assets/mengurai-masalah-sampah-dan-langkah-untuk-mengatasinya/1.0.png)`, backgroundColor: "#d0d1c3" }}>
      <div className="relative flex items-center justify-center z-10 pt-6">
        <motion.div
          initial={{ opacity: 0, y: -20 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: .6 }} viewport={{ once: true }}
          className={`text-center text-4xl md:text-6xl xl:text-7xl py-2 font-extrabold flex flex-col space-y-2 bg-no-repeat text-transparent bg-clip-text`} style={{ backgroundImage: `url('/assets/mengurai-masalah-sampah-dan-langkah-untuk-mengatasinya/1.2.png')` }}>
      ...
      <div className="relative flex items-center flex-col justify-center w-full space-y-12 md:space-y-24 max-w-5xl mx-auto z-10 pt-6">
        <motion.div
          initial={{ opacity: 0, y: -20 }}
          whileInView={{ opacity: 1, y: 0 }}
            ...
            <div className="flex-1">
              <ul className="space-y-4 text-lg md:text-xl lg:text-2xl">
                <motion.li
                  ...
                  <div className="flex-1 text-black">Mengaku selalu membuang sampah di tempatnya</div>
                </motion.li>
                <motion.li
                  initial={{ opacity: 0, x: 20 }} viewport={{ once: true }}
                  ...
                  <div className="flex-1 text-black">Tidak selalu membuang sampah pada tempatnya</div>
                </motion.li>
                <motion.li
                  initial={{ opacity: 0, x: 20 }} viewport={{ once: true }}
                  ...
                  <div className="flex-1 text-black">Tidak pernah membuang sampah di tempatnya</div>
                </motion.li>
              </ul>
            </div>
          </div>
                ...
                {Object.keys(menkes).map((key, index) => (
                  <div key={index} className="flex lg:flex-row flex-col w-full items-start lg:items-center space-x-0 lg:space-x-6 space-y-2 lg:space-y-0">
                    <div className="w-full lg:w-1/2 relative flex items-center justify-center lg:space-x-3 space-x-0 ">
                      <div className="size-24 lg:size-16 -mr-10 z-10">
                        <img src={`/assets/mengurai-masalah-sampah-dan-langkah-untuk-mengatasinya/1.4.${index}.png`} alt="" className="h-full w-full object-contain" />
                      </div>
                      <div className="h-12 lg:h-8 w-full">
                      ...
                      <MotionNumberFlow willChange
                        ...
                        suffix="%"
                        className="text-2xl font-bold w-[100px] text-right text-black"
                        format={{ style: 'decimal' }}
                        //@ts-ignore
                        style={{ '--number-flow-char-height': '0.85em', '--number-flow-mask-height': '0.3em' }}
                        // Important, see note below:
                    ...
                    <motion.div viewport={{ once: true }} initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: .4, delay: index * .1 }} className="text-lg md:text-xl xl:text-2xl w-full lg:w-1/2 text-black">{labels[key]}</motion.div>
                  </div>
                ))}
              </div>
            </MotionConfig>
        ...
        <motion.div
          ...
          className="relative bg-black/50 mt-22! md:mt-auto py-6 px-10 w-fit md:rounded-lg max-w-3xl mx-auto pb-12 flex items-center justify-center">
      ...
      <div className="absolute h-[50%] -mt-6! bg-linear-to-b from-[#efeff0] w-full top-0 z-1" />
      <div className="absolute h-[30%] -mt-6! bg-linear-to-t from-black/80 w-full bottom-0 z-1" />
      <div className="absolute z-0 bottom-0 h-[50%] w-full">
        <div className="absolute inset-6  h-full w-full top-0 left-0 right-0" />
        <img src={`/assets/mengurai-masalah-sampah-dan-langkah-untuk-mengatasinya/1.5.png`} alt="Background" className="w-full mask-t-from-50% object-cover object-bottom h-full opacity-50" />
      </div>
      <img src={`/assets/mengurai-masalah-sampah-dan-langkah-untuk-mengatasinya/footer.png`} className="z-50 relative bottom-0 h-full w-fit mx-auto" />
    </div >
  );
